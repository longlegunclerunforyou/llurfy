            ...
            <div class="panel panel-info"> 
              ...
              <div class="panel-body">
                  ...
                  <?php              
                      ...
                      $rows=mysql_query("select * from job WHERE posterid= '$id'");
                      ...
                      {
                      $pre=$page-1;                                  //上一頁公式
                      echo "<li><a href='postedjob.php?page=$pre'>&laquo;</a></li>";
                      ...
                      for ($i=1;$i<=$show;$i++)
                      {
                        ...
                        {
                          echo "<li><a href=postedjob.php?page=$i>$i</a></li>";
                        }
                      }
                      ...
                      {
                       $next=$page+1;                                      //下一頁公式
                       echo "<li><a href='postedjob.php?page=$next'>&raquo;</a></li>";
                      ...
                      if($_COOKIE["cuk"] == 0) //不限時間
                        {
                            $sql="select * from job WHERE posterid= '$id' order by jobid desc limit $start,5";  //±qguestbookÅª¨ú¸ê®Æ¨Ã¨ÌnoÄæ¦ì°µ»¼´î±Æ§Ç
                            echo "<script>document.getElementById('serchid').value = '不限時間▼'</script>";
                        }
                        else if($_COOKIE["cuk"] == 1) //日
                        {
                            $sql="select * from job WHERE posterid= '$id' and TO_DAYS(NOW()) - TO_DAYS(posttime) <= 1 order by jobid desc limit $start,5";  //±qguestbookÅª¨ú¸ê®Æ¨Ã¨ÌnoÄæ¦ì°µ»¼´î±Æ§Ç
                            echo "<script>document.getElementById('serchid').value = '本日▼'</script>";
                        }
                        else if($_COOKIE["cuk"] == 2) //一星期
                        {
                            $sql="select * from job WHERE posterid= '$id' and TO_DAYS(NOW()) - TO_DAYS(posttime) <= 7 order by jobid desc limit $start,5";  //±qguestbookÅª¨ú¸ê®Æ¨Ã¨ÌnoÄæ¦ì°µ»¼´î±Æ§Ç
                            echo "<script>document.getElementById('serchid').value = '一星期內▼'</script>";
                        }
                        else if($_COOKIE["cuk"] == 3) //一個月
                        {
                            $sql="select * from job WHERE posterid= '$id' and TO_DAYS(NOW()) - TO_DAYS(posttime) <= 30 order by jobid desc limit $start,5";  //±qguestbookÅª¨ú¸ê®Æ¨Ã¨ÌnoÄæ¦ì°µ»¼´î±Æ§Ç
